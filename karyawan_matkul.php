...
<body>
	<div class="container">
		...
		<div class="content">
			<form method="post" action="karyawan_matkul_process.php?idmatkul=<?php echo $idmatkul?>&idkp=<?php echo $kp ?>">
			
			<div class="presensi-dosen">
			...
			if($row['status'] == "Unavailable"){
				echo "&nbsp&nbsp&nbsp<button class='btn-presensi' type='submit' name='btnsubmit' value='checkin'>Check In</button>";
			}else{						
				echo "&nbsp&nbsp&nbsp".$row['kode'];
				echo "&nbsp&nbsp&nbsp<button class='btn-presensi' type='submit' name='btnsubmit' value='checkout'>Check Out</button>";	
			}
					$sql = "SELECT mk.status, mk.kode, mb.kp from matakuliahs_kp mk inner join matakuliahs_buka mb on mb.id = mk.matakuliahs_buka_id where matakuliahs_id = $idmatkul and matakuliahs_buka_id = $kp";
					

					echo "<div class='btn-presensi hapus'><a href='karyawan_matkul_hapus.php?idmatkul=".$idmatkul."&idkp=".$kp."' >Hapus Presensi</a></div>";
				?>
			</div>
		...
		<?php  
			...
			$next_date = date('Y-m-d', strtotime("monday", strtotime($kuliah1)));
		...
		?>
		<div class="simpan">
			<button class="btn-presensi" type="submit" name="btnsubmit" value="presensi">SIMPAN</button>				
		</div>
		</form>
		</div>
		<div class="footer">
